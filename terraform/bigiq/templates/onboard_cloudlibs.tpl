...
#
# inputs
#args="--cloud azure --log-level debug --data-interface eth1 --license $bigIqLicenseKey1 --ntp $ntpServer --timezone $timeZone --create-license-pool $licensePoolKeys --create-reg-key-pool $regPoolKeys --big-iq-password-data-uri file:///mnt/cloudTmp/.bigiq_pass --big-iq-password $adminPassword--big-iq-master-key $masterKey --fcl-tag $f5CloudLibsTag --fcl-cloud-tag $f5CloudLibsAzureTag --vlan 'n:internal,nic:1.1' --self-ip \"n:internal_self,a:$intSubnetPrivateAddress,v:internal,i:eth1\" --usage-analytics 'send:'$allowUsageAnalytics r: $location cI:$subscriptionID dI:$deploymentId cN:azure,lT:byol,bIV:6.0.0,tN:f5-existing-stack-byol-2nic-bigiq,tV:4.3.0"
#help=true
log_level="debug"
cloud="azure"
skip_verify=true
license="$bigIqLicenseKey1"
ntp="$ntpServer"
timezone="$timeZone"
data_interface="eth1"
usage_analytics="$allowUsageAnalytics"
vlan="n:internal,nic:1.1"
self_ip="n:internal_self,a:$intSubnetPrivateAddress,v:internal,i:eth1"
discovery_address="$intSubnetPrivateAddress"
license_pool="$licensePoolKeys"
reg_key_pool="$regPoolKeys"
fcl_tag="$f5CloudLibsTag"
fcl_cloud_tag="$f5CloudLibsAzureTag"
bigiq_password="$adminPassword"
bigiq_master_key="$masterKey"
password_data_uri="file:///mnt/cloudTmp/.bigiq_pass"
master=true
big_iq_failover_peer_ip="$2"
tag_value="tN:f5-existing-stack-byol-2nic-bigiq,tV:4.3.0"
vip_allocation_id="$2"
private_ip="$intSubnetPrivateAddress"
associate_eni="$2"
associate_intf="$2"
dissociate_intf="$2"
#
...
read -r -d '' USAGE << EOM
    Usage: $0
        ...
        --master                    Indicates that this instance is master (used in BIG-IQ clustering) use with --big-iq-failover-peer-ip option
...
[[ ! -z "$master" ]] && config_params+=" --master $master "
...
## Signal
mkdir -p /tmp/f5-cloud-libs-signals
touch "/tmp/f5-cloud-libs-signals/ONBOARD_DONE"
...
## Cluster
if [[ -n "$master" && -n "$big_iq_failover_peer_ip" ]] ; then
    echo "Configuring cluster..."
    cluster_args="--cloud $cloud --master --big-iq-failover-peer-ip $big_iq_failover_peer_ip"
